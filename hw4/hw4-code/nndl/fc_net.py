...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(self.num_layers):
      ...
      if (i == 0):
        self.params[W_name] = np.random.normal(loc=0, scale=weight_scale, size=(input_dim, hidden_dims[i]))
        ...
        self.params[W_name] = np.random.normal(loc=0, scale=weight_scale, size=(hidden_dims[i-1], num_classes))
        self.params[b_name] = np.zeros(num_classes)
      else:
        self.params[W_name] = np.random.normal(loc=0, scale=weight_scale, size=(hidden_dims[i-1], hidden_dims[i]))
        self.params[b_name] = np.zeros(hidden_dims[i])    
        if (self.use_batchnorm):
          self.params[gamma_name] = np.ones(hidden_dims[i])
          self.params[beta_name] = np.zeros(hidden_dims[i])
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if (i == 0):
        if (self.use_batchnorm):
          h, cache = affine_batchnorm_relu_forward(X, self.params[W_name], self.params[b_name], self.params[gamma_name], self.params[beta_name], self.bn_params[i])
        else:
          h, cache = affine_relu_forward(X, self.params[W_name], self.params[b_name])
        if (self.use_dropout):
          h, cache_dropout = dropout_forward(h, self.dropout_param)

      elif (i == self.num_layers-1):
        h, cache = affine_forward(h, self.params[W_name], self.params[b_name])
      else:
        if (self.use_batchnorm):
          h, cache = affine_batchnorm_relu_forward(h, self.params[W_name], self.params[b_name], self.params[gamma_name], self.params[beta_name], self.bn_params[i])
        else:
          h, cache = affine_relu_forward(h, self.params[W_name], self.params[b_name])
        if (self.use_dropout):
          h, cache_dropout = dropout_forward(h, self.dropout_param)
      h_list.append(h)
      cache_list.append(cache)
      if (self.use_dropout):
          cache_dropout_list.append(cache_dropout)
    ...
    for i in range(self.num_layers-1, -1, -1):
      ...
      if (i == self.num_layers-1):
        dh, dw, db = affine_backward(dz, cache_list[i])
      else:
        if (self.use_dropout):
          dh = dropout_backward(dh, cache_dropout_list[i])
        if (self.use_batchnorm):
          dh, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(dh, cache_list[i])
          grads[gamma_name] = dgamma
          grads[beta_name] = dbeta
        else:
          dh, dw, db = affine_relu_backward(dh, cache_list[i])
