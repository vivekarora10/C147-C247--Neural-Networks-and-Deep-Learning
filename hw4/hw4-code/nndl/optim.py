...
def rmsprop(w, dw, config=None):
  ...
  next_w = w - np.multiply(dw, config['learning_rate'] / (np.sqrt(config['a']) + config['epsilon']))
...
def adam(w, dw, config=None):
  ...
  next_w = w - np.multiply(v_tilda, config['learning_rate'] / (np.sqrt(a_tilda) + config['epsilon']))
