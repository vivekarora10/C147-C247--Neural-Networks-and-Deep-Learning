...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    indicator = lambda x : 1 * (x>0)


    softmax[np.arange(X.shape[0]),y] -= 1
    softmax = softmax / X.shape[0]

    grads['b2'] = np.sum(softmax, axis=0) 
    grads['W2'] = softmax.T @ h1 + reg * W2


    grad_layer1 = np.multiply(indicator(r), softmax @ W2)
    grads['b1'] = np.sum(grad_layer1, axis=0)
    grads['W1'] = grad_layer1.T @ X + reg * W1
